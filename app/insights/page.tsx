...
export default function InsightsPage() {
  ...
  // Collapsed sections — largest is open by default
  const [largestExpanded, setLargestExpanded] = useState(true)
  ...
  const uncategorisedCount = useMemo(
    () => transactions.filter(tx => tx.debit != null && (!tx.category || tx.category === 'Uncategorised')).length,
    [transactions]
  )

  const daysSinceLastImport = useMemo(() => {
    const dates = transactions
      .map(tx => tx.importedAt)
  ...
  // Month-over-month in-progress detection
  const currentYearMonth = getCurrentYearMonth()
  const momInProgress = momMonth1 === currentYearMonth || momMonth2 === currentYearMonth
  const inProgressMonthName = momMonth1 === currentYearMonth
    ? formatYearMonth(momMonth1)
    : formatYearMonth(momMonth2)
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {empty ? (
          <div className="bg-white rounded-xl border border-zinc-200 px-6 py-20 text-center text-sm text-zinc-400">
            No transactions yet. Upload a CSV to see your insights.
          </div>
        ) : (
          <>
            {/* Summary cards */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
              ...
              <div className="bg-white rounded-xl border border-zinc-200 p-5">
                <p className="text-xs font-medium text-zinc-500 mb-1">Saved this period</p>
            ...
            {(() => {
              if (uncategorisedCount > 0) {
                return (
                  <div className="flex items-center justify-between gap-4 bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 mb-8">
                    <p className="text-sm text-amber-900">
                      You have <span className="font-semibold">{uncategorisedCount}</span> transaction{uncategorisedCount !== 1 ? 's' : ''} with no category — they&apos;re not showing in your charts.
                    </p>
                    <Link
                      href="/history"
                      className="shrink-0 px-4 py-2 rounded-lg text-sm font-semibold text-amber-900 bg-amber-100 hover:bg-amber-200 transition-colors whitespace-nowrap"
                    >
                      Categorise now →
                    </Link>
                  </div>
              ...
              if (daysSinceLastImport !== null && daysSinceLastImport >= 30) {
                const weeks = Math.floor(daysSinceLastImport / 7)
                const weeksText = weeks === 1 ? '1 week' : `${weeks} weeks`
                return (
                  <div className="flex items-center justify-between gap-4 bg-slate-50 border border-slate-200 rounded-xl px-5 py-4 mb-8">
              ...
              return (
                <div className="flex items-center justify-between gap-4 rounded-xl px-5 py-4 mb-8 border" style={{ background: '#f0f7e8', borderColor: '#c6e0a4' }}>
                  <p className="text-sm" style={{ color: '#3a5c1a' }}>
                    All transactions categorised. Your charts reflect your full history.
                  </p>
                  <Link
                    href="/"
                    className="shrink-0 px-4 py-2 rounded-lg text-sm font-semibold transition-colors whitespace-nowrap"
                    style={{ background: '#d4edbd', color: '#2d4a14' }}
                  >
            ...
            <div className="bg-white rounded-xl border border-zinc-200 p-5 mb-6">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-1">
                <div>
                  ...
                  {momInProgress && (
                    <p className="text-xs text-zinc-400 mt-0.5">{inProgressMonthName} is in progress — data may be incomplete.</p>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <input
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {/* Largest transactions — open by default */}
